...
#define PHYLIB_BALL_DIAMETER (2*PHYLIB_BALL_RADIUS) // 57
#define PHYLIB_HOLE_RADIUS (2*PHYLIB_BALL_DIAMETER)  //  114
...
typedef enum {  // enums are like grouped constants
    PHYLIB_STILL_BALL = 0,
...
// first - a still ball, has a number (cue ball is zero), and a position on the table
typedef struct {
    unsigned char number;
    phylib_coord pos;
} phylib_still_ball;

// second - a rolling ball, has a number (cue ball is zero), and a position on the table
//          also has a velocity and negative friction (due to friction)
typedef struct {
    unsigned char number;
    phylib_coord pos;
    phylib_coord vel;
...
// third - one of the 6 holes on the table, it has a position
typedef struct {
    phylib_coord pos;
} phylib_hole;

// fourth - a horizontal cushion (cushion along the two short sides), only has an y-coordinate
typedef struct {
    double y;
} phylib_hcushion;

// fifth - a vertical cushion (cushion along the two long sides), only has an x-coordinate
typedef struct {
    double x;
} phylib_vcushion;

/*------ Polymorphic Parent Class of objects on the table ------*/
// first - need a C union to store any of the above classes/structures in the same space
typedef union {
...
// second - structure to identify the class of the object 
typedef struct {
    phylib_obj type;
    phylib_untyped obj;
} phylib_object;
...
int segment_bounce(phylib_table *table, int current_index);
